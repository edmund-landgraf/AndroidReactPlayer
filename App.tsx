...
export async function PlaybackService() {
  ...
  TrackPlayer.addEventListener(Event.RemoteJumpBackward, async event => {
    ...
    await TrackPlayer.seekTo(
      Math.max(0, pos - (event.interval ?? SEEK_SECONDS)),
    );
  });
}
...
export default function App(): React.JSX.Element {
  ...
  const [trackTitle, setTrackTitle] = useState('No file loaded');
...
const styles = StyleSheet.create({
  ...
  loadingText: {color: '#888', fontFamily: 'monospace', fontSize: 14},
  ...
  timeText: {color: '#777', fontSize: 11, fontFamily: 'monospace', width: 36},
  ...
  controlIcon: {fontSize: 22, color: '#CCC'},
  primaryIcon: {color: '#0E0E12', fontSize: 28},
  controlLabel: {
    color: '#555',
    fontSize: 10,
    fontFamily: 'monospace',
